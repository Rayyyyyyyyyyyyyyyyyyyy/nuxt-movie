...
export async function _getMovieApi(url: string, params: any): Promise<any> {
  ...
  return await $fetch(`/api/movie/${url}`, {
    params,
  });
}

export function getMovieApi(url: string, params: any): Promise<any> {
  ...
  if (!promiseCache.has(hash)) {
    const movieApiPromise = _getMovieApi(url, params)
      .then((res) => {
        state.value = res;
        return res;
      })
